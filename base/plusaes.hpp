...
// Link: https://github.com/kkAyataka/plusaes
...
struct enc_data {
    enc_data(size_t len__):
        data(new uint8_t[(len__ * 2) + 1]),
        len(len__) {}
    ~enc_data(void) {
        delete data;
    }
...
static inline enc_data *enc_aes_256_cbc(const void *data, size_t data_len,
                                        const void *key, size_t key_len,
                                        const unsigned char (*iv)[16])
{
    size_t len = (size_t) plusaes::get_padded_encrypted_size(data_len);
    enc_data *ret = new enc_data(len);
    bool need_delete;
    unsigned char *fix_key;

    if (key_len < (256 / 8)) {
        fix_key = new unsigned char[256 / 8];
        memset(fix_key, 0, 256 / 8);
        memcpy(fix_key, key, key_len);
        need_delete = true;
    } else {
        need_delete = false;
    }

    plusaes::encrypt_cbc((const unsigned char *)data, data_len, fix_key, 32, iv,
                         ret->data, ret->len, true);
...
static inline enc_data *enc_aes_256_cbc_noiv(const void *data, size_t data_len,
                                             const void *key, size_t key_len)
{
    ...
    return enc_aes_256_cbc(data, data_len, key, key_len, &iv);
}

static inline enc_data *dec_aes_256_cbc(const void *data, size_t data_len,
                                        const void *key, size_t key_len,
                                        const unsigned char (*iv)[16])
{
    size_t len = (size_t) plusaes::get_padded_encrypted_size(data_len);
    ...
    plusaes::decrypt_cbc((const unsigned char *)data, data_len, fix_key, 32, iv,
                         ret->data, ret->len, &ret->padded_size);
...
static inline enc_data *dec_aes_256_cbc_noiv(const void *data, size_t data_len,
                                             const void *key, size_t key_len)
{
    static const unsigned char iv[16] = {
        0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
    ...
    return dec_aes_256_cbc(data, data_len, key, key_len, &iv);
}
